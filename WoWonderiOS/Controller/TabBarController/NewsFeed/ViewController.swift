...
class ViewController: UIViewController,FilterBlockUser,UISearchBarDelegate,editPostDelegate,loadEventDelegate,createLiveDelegate, UITabBarControllerDelegate {
    ...
    var off_set = "0"
    var flagTemp = true
    static var pushToController : ((_ controller: String, _ storyboard: Storyboards) -> Void)? = nil
...
//        self.getnewpost()
    ...
    private func setUpClouser(){
        ViewController.pushToController = {[weak self] (controller,storyboard) in
            guard let self = self else {return}
            if controller == "CreateRoom" {
                self.createRoomPressed()
            }
            if controller == ViewController.id{
    ...
    private func createSearchBar(){
        if let textfield = self.searchBar.value(forKey: "searchField") as? UITextField {
            ...
            textfield.attributedPlaceholder = NSAttributedString(string: "\(" ")\(NSLocalizedString("Search...", comment: "Search..."))", attributes:[NSAttributedString.Key.foregroundColor: UIColor.black])
...
//        self.getnewpost()
    ...
    func createRoomPressed(){
        let alert = UIAlertController(title: "Select Room type", message: NSLocalizedString("", comment: ""), preferredStyle: .actionSheet)
        let storyboard = UIStoryboard(name: "GroupsAndPages", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "CreateGroupVC") as! CreateGroupController
        vc.isHome = 1
        vc.callBack = { [weak self] groupData  in
            self?.gotoGroupController(groupData: groupData)
        }
        ...
        alert.addAction(UIAlertAction(title: NSLocalizedString("Post Room", comment: "Post Room"), style: .default, handler: { (_) in
            vc.modalPresentationStyle = .fullScreen
            vc.modalTransitionStyle = .coverVertical
            self.present(vc, animated: true, completion: nil)
        }))
        alert.addAction(UIAlertAction(title: NSLocalizedString("PodCast Room", comment: "PodCast Room"), style: .default, handler: { (_) in
            vc.modalPresentationStyle = .fullScreen
            vc.modalTransitionStyle = .coverVertical
            self.present(vc, animated: true, completion: nil)
        }))
        alert.addAction(UIAlertAction(title: NSLocalizedString("Live Audio Room", comment: "Live Audio Room"), style: .default, handler: { (_) in
            vc.modalPresentationStyle = .fullScreen
            vc.modalTransitionStyle = .coverVertical
            self.present(vc, animated: true, completion: nil)
        }))
        alert.addAction(UIAlertAction(title: NSLocalizedString("Live Video Room", comment: "Live Video Room"), style: .default, handler: { (_) in
            vc.modalPresentationStyle = .fullScreen
            vc.modalTransitionStyle = .coverVertical
            self.present(vc, animated: true, completion: nil)
        }))
    ...
    private func gotoGroupController(groupData:[String:Any]){
        let storyboard = UIStoryboard(name: "GroupsAndPages", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "GroupVC") as! GroupController
        if let groupId = groupData["group_id"] as? String{
            vc.groupId = groupId
        }
        if let groupName = groupData["group_name"] as? String{
            vc.groupName = groupName
        }
        if let groupTitle = groupData["group_title"] as? String{
            vc.groupTitle = groupTitle
        }
        if let groupIcon = groupData["avatar"] as? String{
            vc.groupIcon = groupIcon
        }
        if let groupCover = groupData["cover"] as? String{
            vc.groupCover = groupCover
        }
        if let groupcategory = groupData["category"] as? String{
            vc.category = groupcategory
        }
        if let about = groupData["about"] as? String{
            vc.aboutGroup = about
        }
        if let categoryId = groupData["category_id"] as? String{
...
//                        self?.newsFeedArray.sorted(by: { _,_ in boosted == 1 })
    ...
    @IBAction func notificationPressed(_ sender: Any) {
        let Storyboards = UIStoryboard(name: "Main", bundle: nil)
        let vc = Storyboards.instantiateViewController(withIdentifier: "NotificationVC") as! NotificationVC
        self.navigationController?.pushViewController(vc, animated: true)
    }
