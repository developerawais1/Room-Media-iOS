...
class GetUserListManager{
    ...
    func getUserList(user_id: String, session_Token: String, completionBlock: @escaping (_ Success:GetUserListModel.GetUserListSuccessModel?,_ RoomName:String?,_ Call_Id:String?,_ senderName:String?,_ profileImage:String?,_ type:String?,_ accessToken2:String?,_ AuthError:GetUserListModel.GetUserListErrorModel?,_ ServerKeyError:GetUserListModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        AF.request("\(API.GetUserList_Constants_Methods.GET_USER_LIST_API)", method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            print(params)
            if (response.value != nil){
                ...
                if apiStatus is Int{
                    ...
                    if self.result1 != nil{
                        completionBlock(result,self.result1?.data?.roomName,self.result1?.data?.id,self.result1?.name,(self.result1?.avatar)!, self.result1?.data?.type,nil, nil, nil, nil)
                        
                    }else if self.result2 != nil{
                        completionBlock(result,self.result2?.data?.roomName,self.result2?.data?.id,self.result2?.name,(self.result2?.avatar)!, "video",self.result2?.data?.accessToken,nil, nil, nil)
                        
                    }else if self.result3 != nil{
                        completionBlock(result,self.result3?.data?.roomName,self.result3?.data?.id,self.result3?.name,self.result3?.avatar, "audio",self.result3?.data?.accessToken,nil, nil, nil)
                    }else{
                        completionBlock(result,nil,nil,nil, nil,nil, nil, nil,nil,nil)
                    }
