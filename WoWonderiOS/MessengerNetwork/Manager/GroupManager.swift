...
class GroupChatManager{
    ...
    func createGroup(session_Token: String,groupName:String,parts:String,type:String,avatar_data:Data?, completionBlock: @escaping (_ Success:MessengerCreateGroupModel.MessengerCreateGroupSuccessModel?,_ AuthError:MessengerCreateGroupModel.CreateGroupErrorModel?,_ ServerKeyError:MessengerCreateGroupModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        }, to: API.GROUP_CHATS_METHODS.CREATE_GROUP_API + "\(session_Token)", method: .post, headers: headers).response { (result) in
            ...
            case .success(let response):
                if (response != nil){
                    ...
                    if apiStatus is Int{
                        ...
                        let result = try? JSONDecoder().decode(MessengerCreateGroupModel.MessengerCreateGroupSuccessModel.self, from: data!)
                        ...
                        if apiStatusString == "400" {
                            ...
                            let data = try? JSONSerialization.data(withJSONObject: response, options: [])
                            let result = try? JSONDecoder().decode(MessengerCreateGroupModel.CreateGroupErrorModel.self, from: data!)
                            log.error("AuthError = \(result?.errors!.errorText)")
                            completionBlock(nil,result,nil,nil)
                        }else if apiStatusString == "404" {
                            let data = try? JSONSerialization.data(withJSONObject: response, options: [])
                            let result = try? JSONDecoder().decode(MessengerCreateGroupModel.ServerKeyErrorModel.self, from: data!)
                            log.error("AuthError = \(result?.errors!.errorText)")
    ...
    func deleteGroup(group_Id: String, session_Token: String,type:String, completionBlock: @escaping (_ Success:MessengerDeleteGroupModel.MessengerDeleteGroupSuccessModel?,_ AuthError:MessengerDeleteGroupModel.DeleteGroupErrorModel?,_ ServerKeyError:MessengerDeleteGroupModel.ServerKeyErrorModel?, Error?) ->()){
        let convertUserId = Int(group_Id)
        let params = [
            API.Params.session_token : session_Token,
            API.Params.Id : convertUserId,
        ...
        AF.request(API.GROUP_CHATS_METHODS.DELETE_GROUP_CHAT_API + "\(session_Token)" , method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            if (response.value != nil){
                ...
                if apiStatus is Int{
                    log.verbose("apiStatus Int = \(apiStatus)")
                    let data = try! JSONSerialization.data(withJSONObject: response.value, options: [])
                    let result = try! JSONDecoder().decode(MessengerDeleteGroupModel.MessengerDeleteGroupSuccessModel.self, from: data)
                    completionBlock(result,nil,nil,nil)
                }else{
                    let apiStatusString = apiStatus as? String
                    if apiStatusString == "400" {
                        log.verbose("apiStatus String = \(apiStatus)")
                        let data = try! JSONSerialization.data(withJSONObject: response.value, options: [])
                        let result = try! JSONDecoder().decode(MessengerDeleteGroupModel.DeleteGroupErrorModel.self, from: data)
                        log.error("AuthError = \(result.errors!.errorText)")
                        completionBlock(nil,result,nil,nil)
                    }else if apiStatusString == "404" {
                        let data = try! JSONSerialization.data(withJSONObject: response.value, options: [])
                        let result = try! JSONDecoder().decode(MessengerDeleteGroupModel.ServerKeyErrorModel.self, from: data)
                        log.error("AuthError = \(result.errors!.errorText)")
                        completionBlock(nil,nil,result,nil)
                    }
                }
    ...
    func updateGroup(session_Token: String,groupName:String,groupId:String,type:String,avatar_data:Data?, completionBlock: @escaping (_ Success:UpdateGroupModel.CreateGroupSuccessModel?,_ AuthError:UpdateGroupModel.UpdateGroupErrorModel?,_ ServerKeyError:UpdateGroupModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        }, to: API.GROUP_CHATS_METHODS.UPDATE_GROUP_CHAT_API + "\(session_Token)", method: .post, headers: headers).response { (result) in
            switch result.result{
            case .success(let response):
    ...
    func sendGroupChatData(message_hash_id: Int,groupId:String,sendType:String,session_Token: String,type:String,image_data:Data?,video_data:Data?,imageMimeType:String?,videoMimeType:String?,text:String,file_data:Data?,file_Extension:String?,fileMimeType:String?, completionBlock: @escaping (_ Success:SendMessageToGroupModel.SendMessageToGroupSuccessModel?,_ AuthError:SendMessageToGroupModel.SendMessageToGroupErrorModel?,_ ServerKeyError:SendMessageToGroupModel.ServerKeyErrorModel?, Error?) ->()){
        ...
        }, to: API.GROUP_CHATS_METHODS.SEND_MESSAGE_TO_GROUP_CHAT_API + "\(session_Token)", method: .post, headers: headers).response { (result) in
            switch result.result{
            case .success(let response):
                if (response != nil){
                    guard let res = response as? [String:Any] else {return}
