...
class MessengerSessionManager{
    ...
    static let instance = MessengerSessionManager()
    
    func getSession( session_Token: String,type: String,completionBlock :@escaping (_ Success:MessengerSessionModel.SessionSuccessModel?,_ AuthError:MessengerSessionModel.SessionErrorModel?,_ ServerKeyError:MessengerSessionModel.ServerKeyErrorModel?, Error?)->()){
        ...
        AF.request(API.SESSION_API.GET_SESSIONS_API + "\(session_Token)", method: .post, parameters: params, encoding:URLEncoding.default , headers: nil).responseJSON { (response) in
            if (response.value != nil){
                ...
                if apiStatus is Int{
                    ...
                    let result = try? JSONDecoder().decode(MessengerSessionModel.SessionSuccessModel.self, from: data!)
