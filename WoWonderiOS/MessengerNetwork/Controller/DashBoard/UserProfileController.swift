...
class UserProfileController: BaseVC {
    ...
    override func viewDidLoad() {
        ...
        if userData?.status == "0"{
            self.statusLabel.text = NSLocalizedString("Hi! there i am using Jointium", comment: "Hi! there i am using Jointium")
        }
        else{
        ...
        if self.userData?.isFollowing == 1{
            ...
            self.followBtn.backgroundColor = UIColor.hexStringToUIColor(hex: ControlSettings.appMainColor)
    ...
    private func getUserData(user_ID: String){
        if Connectivity.isConnectedToNetwork(){
            GetUserDataManager.instance.getUserData(user_id: user_ID, session_Token: UserData.getAccess_Token() ?? "", fetch_type: API.Params.User_data) { (success, sessionError, serverError, error) in
                if success != nil {
                    Async.main({
    ...
    private func  followRequest(){
        if Connectivity.isConnectedToNetwork(){
            Async.main({
                ...
                FollowingManager.instance.followUnfollow(user_id: self.user_data?.userID ?? "", session_Token: UserData.getAccess_Token() ?? "", completionBlock: { (success, sessionError, serverError, error) in
                    if success != nil{
                        Async.main({
                            self.dismissProgressDialog {
                                ...
                                if success?.followStatus  ?? "" == "followed"{
                                    ...
                                    //                                    self.followBtn.setImage(#imageLiteral(resourceName: "tick"), for: .normal)
                                    //                                    self.followBtn.backgroundColor = UIColor.hexStringToUIColor(hex: "mainCol")
                                    //                                    self.view.makeToast(success?.followStatus ?? "")
    ...
    @IBAction func Message(_ sender: Any) {
        if self.isFollowing == 1{
            ...
            vc!.followingUserObject = self.userData!
