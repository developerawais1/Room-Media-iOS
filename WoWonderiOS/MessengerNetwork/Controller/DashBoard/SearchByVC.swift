...
class SearchByVC: UIViewController {
    ...
    @IBAction func genderPressed(_ sender: Any) {
        let vc = R.storyboard.chat.selectCategoryVC()
        vc?.index = 0
        vc?.delegate = self
    ...
    func setupUI(){
        ...
        self.genderLabel.text = AppInstance.instance.profile?.userData?.genderText ?? "All"
        self.profilePictureLabel.text = AppInstance.instance.profile?.userData?.eProfileWallPost ?? "All"
        self.statusLabel.text = AppInstance.instance.profile?.userData?.status ?? "All"
...
extension SearchByVC:GenderDelegate{
    func selectGender(text: String, Index: Int) {
        if Index == 0{
            ...
            AppInstance.instance.profile?.userData?.genderText = self.genderLabel.text
            
        }else if Index == 1{
            self.profilePictureLabel.text = text
            AppInstance.instance.profile?.userData?.eProfileWallPost = self.profilePictureLabel.text
        }else{
            self.statusLabel.text = text
            AppInstance.instance.profile?.userData?.status = self.statusLabel.text
        }
    }
}
