...
class SelectContactVC: BaseVC {
    ...
    func setupUI(){
        ...
        log.verbose("AppInstance.instance.profile?.userData?.userID = \(AppInstance.instance.profile?.userData?.userID)")
        log.verbose("UserData.getAccess_Token() = \(UserData.getAccess_Token())")
    ...
    private func fetchData(){
        ...
        Async.background({
            SelectContactManger.instance.getContactList(user_id: UserData.getUSER_ID() ?? "", session_Token: UserData.getAccess_Token() ?? "",  list_type: "all") { (success, sessionError, serverError, error) in
                if success != nil{
                    Async.main({
